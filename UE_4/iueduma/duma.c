...
void * _duma_allocate(size_t alignment, size_t userSize, int protectBelow, int fillByte, int protectAllocList, enum _DUMA_Allocator allocator, enum _DUMA_FailReturn fail  DUMA_PARAMLIST_FL)
{
  ...
  _duma_s.numAllocs++;
  /* if (_duma_s.SHOW_ALLOC) always do this [PS]*/
  if (_duma_s.numAllocs>1) /* needed to suppress bougs output due to selftest!! see duma.c:599 */
  {
    #ifndef DUMA_NO_LEAKDETECTION
      DUMA_Print("\nDUMA: Allocating %d bytes at %s(%i).\n", (DUMA_SIZE)userSize, filename, lineno);
    #else
      DUMA_Print("\nDUMA: Allocating %d bytes.\n", (DUMA_SIZE)userSize);
    #endif
    if ( 0 == userSize )
      DUMA_Print(" This is ANSI conform but probably a bug. See DUMA_ALLOW_MALLOC_0.\n");
          ...
          if (first)
          {
             char *probtxt = getenv("DUMA_MALLOC_ERROR");
             time_t t;
             time(&t);
             first = 0;
             srand(t);
             ...
             fprintf(stderr,"\nDuma: Using error probability %d%%\n", prob);
          }
          // fprintf(stderr,"\033[01;34mDuma: Requesting %d bytes\033[0m\n", userSize);
          r = (rand()/(RAND_MAX+1.0))*100;
          // printf("r = %ld\n", r);
          if (r < prob)
          {
             fprintf(stderr,"\033[01;31mDuma: Returning 0\033[0m\n");
             return 0;
          }
  // end of edit
  }
...
void _duma_deallocate(void * address, int protectAllocList, enum _DUMA_Allocator allocator  DUMA_PARAMLIST_FL)
{
  ...
  if ( !(slot = slotForUserAddress(address)) )
  {
    ...
    else
    {
      ...
      DUMA_Print("DUMA_Warning: free(%a): address not from DUMA or already freed.\n", (DUMA_ADDR)address);
      return;
#else
