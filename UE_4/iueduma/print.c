...
{
  char buffer[STRING_BUFFER_SIZE],b2[STRING_BUFFER_SIZE];
  int len, fd;
  va_list  args;

  va_start(args, pattern);
  len = DUMA_vsprintf(b2, pattern, args);
  va_end(args);
#ifndef WIN32
  sprintf(buffer,"\x1B[34m%s\x1B[0m",b2);
#else
  sprintf(buffer,"%s",b2);
#endif
  len=strlen(buffer);
  ...
  {
#if defined(WWIN32) && !defined(__CYGWIN__)
